"""
app/main.py
YOLO Inference API - FastAPI + Ultralytics YOLOv8 (Aula 2),
acrescida dos logs estruturados JSON da Aula 3.

Endpoints:
    GET  /health          - status do servico e do modelo
    GET  /metrics         - contadores e latencia media
    POST /predict         - inferencia em uma imagem (base64 ou URL)
    POST /predict/image   - inferencia devolvendo a imagem anotada
    POST /predict/batch   - inferencia em varias imagens
"""
...
def _load_image_from_request(request: PredictRequest) -> np.ndarray:
    """Le a imagem a partir de base64 ou URL publica, sempre em RGB."""
...
def predict(request: PredictRequest):
    ...
    try:
        ...
        log_event(
            "predict_error", level="ERROR", request_id=request_id, reason=str(e)
        )
...
def predict_image(request: PredictRequest):
    """Executa a inferencia e retorna a imagem anotada em JPEG (RGB)."""
    _metrics["total"] += 1
    try:
        ...
        # 3. Salva via PIL (RGB nativo, sem a conversao indevida do OpenCV)
...
def predict_batch(request: BatchPredictRequest):
    request_id = str(uuid.uuid4())[:8]
    log_event("batch_start", request_id=request_id, images=len(request.images_base64))
