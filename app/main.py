...
def _run_inference(image_np: np.ndarray, model_name: str, confidence: float) -> PredictResponse:
    ...
    for r in results:
        for box in r.boxes:
            # Ajusta as coordenadas do espaço letterboxed de volta ao espaço
            # da imagem original -- sem isso, os bboxes retornados pela API
            # ficam deslocados sempre que houver padding
...
def predict(request: PredictRequest):
    ...
    try:
        ...
        log_event("predict_error", level="WARN" if e.status_code == 422 else "ERROR",
                  request_id=request_id, reason=e.detail)
        raise
