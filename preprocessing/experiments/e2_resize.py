...
if __name__ == "__main__":
    ...
    variants = [
        (None, "E2-baseline"),
        (preproc_naive_resize, "E2-A: resize ingênuo"),
        (preproc_letterbox, "E2-B: letterbox"),
    ]
    ...
    print("\n--- Resumo E2 (dataset completo, 3 classes) ---")
    b = results[0]['map50']
    for r in results[1:]:
        print(f"  {r['label']:30s} mAP@0.5={r['map50']:.4f}  delta={r['map50']-b:+.4f}")

    print("\n--- Resumo E2 (apenas Pessoa — sinal real) ---")
    bp = results_person[0]['map50']
    for r in results_person[1:]:
        print(f"  {r['label']:30s} mAP@0.5={r['map50']:.4f}  delta={r['map50']-bp:+.4f}")
