...
def benchmark_filter_cost(n_frames: int = 200):
    ...
    print(f"\n--- Custo por filtro (média de {n_frames} frames 640×480) ---")
    for nome, fn in fns:
...
if __name__ == "__main__":
    ...
    variants = [
        (None, "E3-baseline"),
        (preproc_rgb_only, "E3-A: RGB apenas (sem filtro)"),
        (preproc_gauss_33, "E3-B: GaussianBlur 3×3"),
        (preproc_gauss_55, "E3-C: GaussianBlur 5×5"),
        (preproc_gauss_77, "E3-D: GaussianBlur 7×7"),
        (preproc_median_3, "E3-E: medianBlur k=3"),
    ]
    ...
    print("\n--- Resumo E3 (dataset completo, 3 classes) ---")
    b = results[0]['map50']
    for r in results[1:]:
        print(f"  {r['label']:35s} mAP={r['map50']:.4f}  delta={r['map50']-b:+.4f}")

    print("\n--- Resumo E3 (apenas Pessoa — sinal real) ---")
    bp = results_person[0]['map50']
    for r in results_person[1:]:
        print(f"  {r['label']:35s} mAP={r['map50']:.4f}  delta={r['map50']-bp:+.4f}")
