...
def main():
    ...
    print(f"[INFO] Abrindo câmera CSI via rpicam-vid (câmera {args.device}) "
    ...
    while frame_count < args.frames:
        ...
        if frame_count % 10 == 0:
            print(f"{frame_count:>6} | {cap_ms:>7.1f}ms | {infer_ms:>9.1f}ms | "
                  f"{cycle_ms:>7.1f}ms | {fps_inst:>8.1f}")
