...
class RealtimeDetector:
    ...
    def process(self, frame: np.ndarray) -> np.ndarray:
        ...
        if self._frame_idx % self.infer_every == 0:
            # Pré-processamento pelo módulo central (letterbox preserva a
            # proporção -- o resize ingênuo anterior distorcia os objetos)
...
def main():
    ...
    try:
        ...
        if writer:
            writer.release()
        # cv2.destroyAllWindows() exige suporte a GUI (highgui). Com o build
        # headless (opencv-python-headless, usado neste projeto), essa chamada
        # falha mesmo sem nenhuma janela ter sido criada -- só faz sentido
        # quando --no-display não foi usado (janela real aberta via imshow).
        if not args.no_display:
