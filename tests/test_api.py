...
Pre-requisito: models/yolov8n.pt presente no sistema de arquivos
(acessivel via /app/models/yolov8n.pt).
"""
...
# Ajusta o PYTHONPATH: raiz do projeto (para "app" ser pacote) e app/
# (para os imports internos de main.py, como "from schemas import ...")
sys.path.insert(0, str(Path(__file__).parent.parent))
sys.path.insert(0, str(Path(__file__).parent.parent / "app"))

os.environ.setdefault("MODEL_NAME", "yolov8n.pt")

from app.main import _decode_image, app  # noqa: E402
...
class TestSmoke:
    ...
    def test_metrics_endpoint_accessible(self):
        """Endpoint /metrics deve estar acessivel."""
        resp = client.get("/metrics")
        assert resp.status_code == 200


# ────────────────────────────────────────────────────────────
# UNIT TESTS - funcoes isoladas
# ────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────
# INTEGRATION TESTS - fluxo completo de inferencia
# ────────────────────────────────────────────────────────────
class TestPredictEndpoint:
    ...
    def test_predict_returns_200(self, zidane_b64):
        resp = client.post(
            "/predict",
            json={"image_base64": zidane_b64, "confidence": 0.3},
        )
        assert resp.status_code == 200

    def test_predict_detects_at_least_one_object(self, zidane_b64):
        """A imagem zidane.jpg deve produzir ao menos 1 deteccao com conf >= 0.3."""
        data = client.post(
            "/predict",
            json={"image_base64": zidane_b64, "confidence": 0.3},
        ).json()
    ...
    def test_predict_detection_fields(self, zidane_b64):
        """Cada deteccao deve ter label, confidence e bbox validos."""
        data = client.post(
            "/predict",
            json={"image_base64": zidane_b64, "confidence": 0.3},
        ).json()
    ...
    def test_predict_missing_input_returns_422(self):
        """Requisicao sem imagem deve retornar HTTP 422."""
